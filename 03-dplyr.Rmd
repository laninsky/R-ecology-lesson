...
-- last time we learned about data frames, and tricky stuff like strings and factors
...
So far we've been using functions built into R
...
One big example of this is how we had to set **`stringsAsFactors`** to **`FALSE`** to avoid our data being coerced/forced this hidden argument to convert our data type. 
...
# compared to read.csv that we've already seen
...
Now we are going to switch gears and use functions provided by tidyr to reshape our data while still following the "tidy data rules" we learned back in the spreadsheet session
...
If we wanted to compare the different mean weight of each species between plots to follow these rules we'd need to create a new table where each row is the plot, species would become the names of column variables and the cells would contain the values of the mean weight observed on each plot.
